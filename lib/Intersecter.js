...
export class Intersecter {
	...
	compareEvents(eventA, eventB) {
		...
		return +eventA.isStart - +eventB.isStart // favor the one that isn't the start
			|| this.compareSegments(eventB.seg.data, eventA.seg.data);
	}
	 ...
	 */
	addEvent(ev) {
		this.events.insertBefore(ev, here => this.compareEvents(ev, here));
	}
	...
	statusFindSurrounding(ev) {
		return this.status.findTransition(here => {
			...
			return this.compareSegments(ev.seg.data, here.seg.data) || -1;
		});
	}
