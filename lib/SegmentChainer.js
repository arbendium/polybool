...
export function SegmentChainer(segments, geo) {
	...
	for (const segb of segments) {
		...
		/** @type {{ index: number, matchesHead: boolean, matchesPt1: boolean }[]} */
		const matches = [];
		...
		for (let i = 0; i < chains.length && matches.length < 2; i++) {
			...
			if (geo.isEqualVec2(tail, pt1) && chainFill === fill) {
				matchesHead = false;
				matchesPt1 = true;
			} else if (geo.isEqualVec2(tail, pt2) && chainFill !== fill) {
				matchesHead = false;
				matchesPt1 = false;
			} else if (geo.isEqualVec2(head, pt1) && chainFill !== fill) {
				matchesHead = true;
				matchesPt1 = true;
			} else if (geo.isEqualVec2(head, pt2) && chainFill === fill) {
				matchesHead = true;
				matchesPt1 = false;
			} else {
				continue;
		...
		if (matches.length === 0) {
			...
			if (finalChain.length > 0 && geo.isEqualVec2(segS.p0, segE.p1)) {
			...
			assert(firstMatch.matchesPt1 !== secondMatch.matchesPt1);
